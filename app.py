...
output_file = cv2.VideoWriter('output3.avi', fourcc, 20.0, (640, 480))
...
time.sleep(3)
bg = 0
count=0 #not from wht hat
#Capturing background for 60 frames
for i in range(60):
    ret, bg = cap.read()
#Flipping the background
bg = np.flip(bg, axis=1) # for me try with cv2.flip(bg, 0)
...
while (cap.isOpened()):
    ...
    # lower_red = np.array([0, 120, 70]) #hue, saturation and value for red color
    ...
    # src1, weight of file 1, src2, weightof file 2, scalar to be added
